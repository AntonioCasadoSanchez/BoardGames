...
app.controller("GameController", function($scope, $http) {
	...
	$scope.login = function() {
		if($scope.pwd == $scope.userName + "123"){//eSTA Comprobacion va aqui o la tiene que hacer en la Bdatos?
			var request =new XMLHttpRequest();
			  request.open("GET", "http://localhost:8080/login?userName="+ $scope.userName+"&pwd="+$scope.pwd);
	...
	$scope.login2 = function(){
		if($scope.pwd == $scope.userName + "123"){
			$scope.estado = "El user es: " + $scope.userName + "La pwd es: " + $scope.pwd;
		}
		else{
			$scope.estado = "Error";
		}
